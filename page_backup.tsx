...
export default function StatsPage() {
  ...
  const sidebarLinks = [
    { id: 'arcade' as const, label: 'Arcade', href: '/?tab=arcade', icon: <Gamepad2 size={18} /> },
    { id: 'wallet-stats' as const, label: 'Wallet Stats', href: '/?tab=wallet-stats', icon: <Activity size={18} /> },
    { id: 'daily-streak' as const, label: 'Daily Streak', href: '/?tab=daily-streak', icon: <Flame size={18} /> },
    { id: 'deploy' as const, label: 'Deploy', href: '/?tab=deploy', icon: <Rocket size={18} /> },
    { id: 'leaderboard' as const, label: 'Leaderboard', href: '/?tab=leaderboard', icon: <Award size={18} /> },
  ];

  return (
    <div className={`min-h-screen flex flex-col lg:flex-row relative overflow-hidden font-sans select-none transition-colors duration-300`} style={{background: isDark ? "#0A0F1E" : "#f0f4ff", color: isDark ? "#ffffff" : "#0a0f2e"}}>
      
      <div className="radial-glow-top" />
      <div className="radial-glow-bottom" />

      {/* SIDEBAR */}
      <aside className="hidden lg:flex w-72 backdrop-blur-2xl py-10 px-6 flex-col justify-between z-20 shrink-0 relative transition-colors duration-300" style={{background: isDark ? "rgba(255,255,255,0.01)" : "rgba(255,255,255,0.9)", borderRight: isDark ? "1px solid rgba(255,255,255,0.05)" : "1px solid rgba(0,0,0,0.08)"}}>
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3.5 px-4 py-3.5 rounded-2xl text-xs font-bold uppercase tracking-wider transition-all duration-300 relative ${
                    active
                      ? isDark ? 'text-white bg-gradient-to-r from-white/[0.07] to-transparent border-l-2 border-[#00D1FF]' : 'text-slate-900 bg-[#00D1FF]/10 border-l-2 border-[#00D1FF]'
                      : isDark ? 'text-white/45 hover:text-white hover:bg-white/[0.02]' : 'text-slate-500 hover:text-slate-900 hover:bg-black/[0.04]'
                  }`}
                >
        ...
        <div className={`flex flex-col gap-2 border-t pt-6 font-mono text-[9px] uppercase tracking-widest ${isDark ? "border-white/5 text-white/30" : "border-black/10 text-slate-400"}`}>
          <div className="flex items-center gap-1.5">
      ...
      <main className="flex-1 px-4 md:px-8 lg:px-14 py-8 lg:py-10 flex flex-col gap-8 overflow-x-hidden relative z-10 max-w-7xl mx-auto w-full mb-16 lg:mb-0">
        ...
        {/* Desktop top bar */}
        <div className="hidden lg:flex justify-between items-center backdrop-blur-xl rounded-3xl p-5" style={{background: isDark ? "rgba(255,255,255,0.02)" : "rgba(255,255,255,0.8)", border: isDark ? "1px solid rgba(255,255,255,0.05)" : "1px solid rgba(0,0,0,0.08)"}}>
          <div className="flex items-center gap-3">
            <Activity className="text-[#00D1FF] animate-pulse" size={20} />
            <span className={`text-xs font-mono font-bold tracking-widest uppercase ${isDark ? "text-white/70" : "text-slate-600"}`}>
              Onchain Analytics Telemetry Scanners
            </span>
          </div>
          <div className="flex items-center gap-3">
            <a href="https://x.com/MSGGAMER1091" target="_blank" rel="noopener noreferrer" className=`flex items-center gap-2 border hover:border-[#00D1FF]/30 transition-all duration-300 rounded-2xl px-3 py-1.5 group ${isDark ? "bg-white/5 hover:bg-white/10 border-white/10" : "bg-black/5 hover:bg-black/10 border-black/10"}`>
            ...
            <button
              ...
              className=`w-9 h-9 flex items-center justify-center rounded-xl border transition-all duration-300 ${isDark ? "bg-white/5 border-white/10 hover:border-[#00D1FF]/40 hover:bg-white/10" : "bg-black/5 border-black/10 hover:border-[#00D1FF]/60 hover:bg-black/10"}`
              title={isDark ? 'Switch to Light Mode' : 'Switch to Dark Mode'}
            >
              {isDark ? (
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#00D1FF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/>
                </svg>
              ) : (
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#7B61FF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  const sidebarLinksort React, { useState, useEffect } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { 
  Gamepad2, Activity, Flame, Rocket, Award, Zap, Terminal, ExternalLink, 
  Coins, Compass, Layers, Search, RefreshCw, AlertCircle, Calendar, 
